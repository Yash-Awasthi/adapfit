...
Supabase Postgres when configured, in-memory fallback otherwise.

Follows Supabase Postgres Best Practices:
  - conn-pooling: uses Supabase pooler (transaction mode)
...
class StorageEngine:
    """Async-safe storage with Supabase Postgres or in-memory fallback."""

    def __init__(self):
        self._lock = asyncio.Lock()
        # In-memory state (used when Supabase is not configured)
    ...
    async def get_sleep_logs(self, user_id: str, days: int = 14) -> list[dict]:
        """Get sleep logs for a user over the given number of days."""
        logs = self.recovery_logs.get(user_id, [])
        # Filter logs that have sleep data
        sleep_logs = [
            l for l in logs
            if l.get("wearable_data", {}).get("sleep_duration_hours")
            or l.get("sleep_score")
        ]
        # Also check dedicated sleep storage if it exists
        if hasattr(self, '_sleep_data') and user_id in self._sleep_data:
            sleep_logs.extend(self._sleep_data[user_id][-days:])
    ...
    async def get_stats(self) -> dict:
        return {
            ...
            "backend": "supabase" if _USE_SUPABASE else "in_memory",
        }
